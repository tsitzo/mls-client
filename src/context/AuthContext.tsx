...
export const AuthContextProvider: FC = ({ children }) => {
  const [token, setToken] = useState<string | null>(contextDefaultValue.token);
  const [user, setUser] = useState<User | null>(contextDefaultValue.user);
  const [loading, setLoading] = useState<boolean>(contextDefaultValue.loading);
  const [userLoading, setUserLoading] = useState<boolean>(
    contextDefaultValue.userLoading
  );
  ...
  const register = async (credentials: RegisterCredentials) => {
    ...
    try {
      ...
      await multiSet(
        ["@MLS/Token", response.data.token],
        ["@MLS/User", JSON.stringify(response.data.user)]
      );
  ...
  const logout = async () => {
    try {
      await removeMultiple("@MLS/Token", "@MLS/User");
  ...
  const loadToken = async () => {
    setUserLoading(true);
    try {
      const values: any = await multiGet("@MLS/Token", "@MLS/User");
      const token = values[0][1];
